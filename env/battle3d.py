...
from env.action_space import MultiAgentActionSpace
from env.observation_space import MultiAgentObservationSpace
from env.draw import draw_grid, fill_cell, write_cell_text
...
class Combat(gym.Env):
    ...
    def __init__(self, grid_shape=(15, 15, 15), n_agents=5, n_opponents=5, init_health=3, full_observable=False,
        ...
        self.action_space = MultiAgentActionSpace(
            [spaces.Discrete(7 + self._n_opponents) for _ in range(self.n_agents)])
        ...
        # 5 * 5 * 5 * (type, id, health, cool, x, y, z)
        self._obs_low = np.repeat([-1., 0., 0., -1., 0., 0., 0.], 5 * 5 * 5)
        self._obs_high = np.repeat([1., n_opponents, init_health, 1., 1., 1., 1.], 5 * 5 * 5)
    ...
    def get_agent_obs(self):
        ...
        for agent_i in range(self.n_agents):
            ...
            _agent_i_obs = np.zeros((7, 5, 5, 5))
            for row in range(0, 5):
                for col in range(0, 5):
                    for dep in range(0, 5):
                        if self.is_valid([row + (pos[0] - 2), col + (pos[1] - 2), dep + (pos[2] - 2)]) and (
                                PRE_IDS['empty'] not in self._full_obs[row + (pos[0] - 2)][col + (pos[1] - 2)][dep + (pos[2] - 2)]):
                            x = self._full_obs[row + pos[0] - 2][col + pos[1] - 2][dep + pos[2] -2]
                            _type = 1 if PRE_IDS['agent'] in x else -1
                            _id = int(x[1:]) - 1  # id
                            _agent_i_obs[0][row][col][dep] = _type
                            _agent_i_obs[1][row][col][dep] = _id
                            #                         print('type', type, '_type', _type)
                            _agent_i_obs[2][row][col][dep] = self.agent_health[_id] if _type == 1 else self.opp_health[_id]
                            _agent_i_obs[3][row][col][dep] = self._agent_cool[_id] if _type == 1 else self._opp_cool[_id]
                            _agent_i_obs[3][row][col][dep] = 1 if _agent_i_obs[3][row][col][dep] else -1  # cool/uncool

                            _agent_i_obs[4][row][col][dep] = pos[0] / self._grid_shape[0]  # x-coordinate
                            _agent_i_obs[5][row][col][dep] = pos[1] / self._grid_shape[1]  # y-coordinate
                            _agent_i_obs[6][row][col][dep] = pos[2] / self._grid_shape[2]  # z-coordinate
    ...
    def __create_grid(self):
        _grid = [[[PRE_IDS['empty'] for _ in range(self._grid_shape[1])] for row in range(self._grid_shape[0])] for _ in range(self._grid_shape[2])]
        return _grid
    ...
    def __update_agent_view(self, agent_i):
        self._full_obs[self.agent_prev_pos[agent_i][0]][self.agent_prev_pos[agent_i][1]][self.agent_prev_pos[agent_i][2]] = PRE_IDS['empty']
        self._full_obs[self.agent_pos[agent_i][0]][self.agent_pos[agent_i][1]][self.agent_pos[agent_i][2]] = PRE_IDS['agent'] + str(agent_i + 1)    #在_full_obs上改变智能体位置

    def __update_opp_view(self, opp_i):
        self._full_obs[self.opp_prev_pos[opp_i][0]][self.opp_prev_pos[opp_i][1]][self.opp_prev_pos[opp_i][2]] = PRE_IDS['empty']
        self._full_obs[self.opp_pos[opp_i][0]][self.opp_pos[opp_i][1]][self.opp_pos[opp_i][2]] = PRE_IDS['opponent'] + str(opp_i + 1)

    def __init_full_obs(self):
        """ Each team consists of m = 10 agents and their initial positions are sampled uniformly in a 5 × 5
        square.
        """
        self._full_obs = self.__create_grid()   #   15*15*15的列表，‘empty’填充
        ...
        agent_team_corner = random.randint(0, int(self._grid_shape[0] / 2)), random.randint(0, int(
            self._grid_shape[1] / 2)), random.randint(0, int(self._grid_shape[2] / 2))   #   (x,y,z)地图的左下部分
        agent_pos_index = random.sample(range(25), self.n_agents)   #   [id1,id2]
        ...
        for agent_i in range(self.n_agents):
            pos = [int(agent_pos_index[agent_i] / 5) + agent_team_corner[0],
                   ...
                   agent_pos_index[agent_i] % 5 + agent_team_corner[2]]
            #             [x,y,z]智能体位置
            while True:
                if self._full_obs[pos[0]][pos[1]][pos[2]] == PRE_IDS['empty']:
                ...
                pos = [random.randint(agent_team_corner[0], agent_team_corner[0] + 4),
                       ...
                       random.randint(agent_team_corner[2], agent_team_corner[2] + 4)]
        ...
        while True:
            pos = random.randint(agent_team_corner[0], self._grid_shape[0] - 5),\
                  ...
                  random.randint(agent_team_corner[2], self._grid_shape[2] - 5)
            if self._full_obs[pos[0]][pos[1]][pos[2]] == PRE_IDS['empty']:
                opp_team_corner = pos
                break
        ...
        for opp_i in range(self._n_opponents):
            pos = [int(opp_pos_index[agent_i] / 5) + opp_team_corner[0],
                   ...
                   opp_pos_index[agent_i] % 5 + opp_team_corner[2]]
            while True:
                if self._full_obs[pos[0]][pos[1]][pos[2]] == PRE_IDS['empty']:
                ...
                pos = [random.randint(opp_team_corner[0], opp_team_corner[0] + 4),
                       ...
                       random.randint(opp_team_corner[2], opp_team_corner[2] + 4)]
    ...
    def __update_agent_pos(self, agent_i, move):
        ...
        if next_pos is not None and self._is_cell_vacant(next_pos): #空的才能移动
    ...
    def is_valid(self, pos):    #是否在界内
        return (0 <= pos[0] < self._grid_shape[0]) and (0 <= pos[1] < self._grid_shape[1]) and (0 <= pos[2] < self._grid_shape[2])

    def _is_cell_vacant(self, pos):
        return self.is_valid(pos) and (self._full_obs[pos[0]][pos[1]][pos[2]] == PRE_IDS['empty'])
    ...
    def is_visible(source_pos, target_pos):
        ...
        return (source_pos[0] - 2) <= target_pos[0] <= (source_pos[0] + 2) \
               and (source_pos[1] - 2) <= target_pos[1] <= (source_pos[1] + 2) \
                and (source_pos[2] - 2) <= target_pos[2] <= (source_pos[2] + 2)
    ...
    def is_fireable(source_pos, target_pos):
        ...
        return (source_pos[0] - 1) <= target_pos[0] <= (source_pos[0] + 1) \
               and (source_pos[1] - 1) <= target_pos[1] <= (source_pos[1] + 1) \
                and (source_pos[2] - 1) <= target_pos[2] <= (source_pos[2] + 1)
    ...
    def reduce_distance_move(self, source_pos, target_pos):
        ...
        if source_pos[2] > target_pos[2]:
            _moves.append('UP')
        elif source_pos[2] < target_pos[2]:
            _moves.append('DOWN')
    ...
    def opps_action(self):
        ...
        for opp_i, opp_pos in self.opp_pos.items():
            for agent_i, agent_pos in self.agent_pos.items():
                ...
                distance = abs(agent_pos[0] - opp_pos[0]) + abs(agent_pos[1] - opp_pos[1]) + abs(agent_pos[2] - opp_pos[2])  # manhattan distance
    ...
    def step(self, agents_action):
        ...
        for agent_i, action in enumerate(agents_action):
            if self.agent_health[agent_i] > 0:
                if action > 6:  # attack actions
                    target_opp = action - 7
                    if self.is_fireable(self.agent_pos[agent_i], self.opp_pos[target_opp]) \
                            and opp_health[target_opp] > 0 and self._agent_cool[agent_i]:
                ...
                if action > 6:  # attack actions
                    ...
                    if self.is_fireable(self.opp_pos[opp_i], self.agent_pos[target_agent]) \
                        ...
                        if agent_health[target_agent] == 0:
                            ...
                            self._full_obs[pos[0]][pos[1]][2] = PRE_IDS['empty']
