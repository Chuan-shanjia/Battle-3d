...
class Combat(gym.Env):
    ...
    def get_agent_obs(self):
        ...
        for agent_i in range(self.n_agents):
            ...
            for row in range(0, 5):
                for col in range(0, 5):
                    if self.is_valid([row + (pos[0] - 2), col + (pos[1] - 2)]) and (
                        ...
                        _type = 1 if PRE_IDS['agent'] in x else -1  #1表示格子里是队友
                        _id = int(x[1:]) - 1  # id
            ...
            _agent_i_obs = _agent_i_obs.flatten().tolist()  #不flatten，直接返回(6，5，5)
    ...
    def __update_agent_view(self, agent_i):
        ...
        self._full_obs[self.agent_pos[agent_i][0]][self.agent_pos[agent_i][1]] = PRE_IDS['agent'] + str(agent_i + 1)    #在_full_obs上改变智能体位置
    ...
    def __init_full_obs(self):
        ...
        square.
        """
        self._full_obs = self.__create_grid()   #   15*15的列表，‘empty’填充
        ...
        agent_team_corner = random.randint(0, int(self._grid_shape[0] / 2)), random.randint(0, int(
            self._grid_shape[1] / 2))   #   (x,y)地图的左下部分
