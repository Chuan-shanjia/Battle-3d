from env.modelbattle2d import Combat
...
if __name__ == "__main__":
    ...
    agent1 = PPO(state_dim, hidden_dim, action_dim, actor_lr, critic_lr, lmbda, eps,
                gamma, device)
    agent2 = PPO(state_dim, hidden_dim, action_dim, actor_lr, critic_lr, lmbda, eps,
                gamma, device)
    ...
    with tqdm(total=int(num_episodes),desc='Iteration %d' % num_episodes) as pbar:
        for k in range(num_episodes):
            transition_dict1 = [[] for _ in range(n_agent)]
            transition_dict2 = [[] for _ in range(n_agent)]
            ...
            while not terminal:
                ...
                for i in range(n_agent):
                    a[i] = agent1.take_action(s[i])

                next_s, r, done, info = env.step(a)

                for i in range(n_agent):
                    transition_dict1[i]['states'].append(s[i])
                    transition_dict1[i]['actions'].append(a[i])
                    transition_dict1[i]['next_states'].append(next_s[i])
                    transition_dict1[i]['rewards'].append(
                        r[i] + 100 if info['win'] else r[i] - 0.1)
                    transition_dict1[i]['dones'].append(False)
            ...
            for i in range(n_agent):
                agent1.update(transition_dict1[i])
